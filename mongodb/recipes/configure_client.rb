...
node['deploy'].each do |application, deploy|
  ...
  unless node['mongodb']['replica_set'].empty?
    mongo_config['dbUrl'] = instances.map{|k,v| "#{v['private_ip']}:#{node['mongodb']['port']}/?auto_reconnect=true"}
  else
    mongo_config['dbUrl'] = "#{instances[instances.keys.first]['private_ip'] rescue 'localhost'}:#{node['mongodb']['port']}/?auto_reconnect=true"
  end
